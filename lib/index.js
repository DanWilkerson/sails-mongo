    ...
    this.find(identity, s3q, (err, items) => {
      ...
      gcDatastoreInstance
        ...
        .then(resp => {
          ...
          this.find(identity, s3q, (err, _items) => {
...
/*
  ...
  */
  find: (identity, s3q, done) => {
    ...
    for (key in criteria.where) { query.filter(key, criteria.where[key]); }

    if (criteria.limit) { query.limit(Math.min(criteria.limit, 2147483647)); }
    if (criteria.skip) { return done(new Error('Skip not implemented')); }
    if (criteria.select.join !== '*') query.select([criteria.select]);
